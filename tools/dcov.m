function [d, p_value] =  dcov(dist_X, dist_Y, varargin)
% [dcov, pvalue] =  dcov(dist_X, dist_Y, 'option', option_value)
%
...
%
%   Optional arguments:
%   ddov(dist_X, dist_Y, 'random', num_shuffles) selects the
%       random permutation test (default).  Choose the num_shuffles (1x1)
%       for the test (default = 1000).
%   dcov(dist_X, dist_Y, 'shifted', time_window) selects the
...
%
% OUTPUT:
%   dcov: scalar, the distance covariance
...
%   bcowley, 7/21/2015
    ...
    % compute the dcov of the original data
    d = dcov_plain(dist_X,dist_Y);
    
    % compute distribution of shuffled data (permutation test)
    if (strcmp(permute_type, 'random'))
        % RANDOM PERMUTATION TEST
        
        dcov_shuffled = zeros(1, num_shuffles);
        
        parfor ishuffle = 1:num_shuffles
            dist_Y_shuffled = permute_matrix(dist_Y);
            dcov_shuffled(ishuffle) = dcov_plain(dist_X, dist_Y_shuffled);
        end
        
        % compute p_value
        p_value = sum(dcov_shuffled >= d) / num_shuffles;
    ...
    elseif (strcmp(permute_type, 'shifted'))
        ...
        dcov_shuffled = zeros(1, length(time_window));
    
        parfor itime = 1:length(time_window)
            dist_Y_shuffled = shift_matrix(dist_Y, time_window(itime));
            dcov_shuffled(itime) = dcov_plain(dist_X, dist_Y_shuffled);
        end
        
        p_value = sum(dcov_shuffled >= d) / length(time_window);
...
function d = dcov_plain(dist_X,dist_Y)
% helper function
